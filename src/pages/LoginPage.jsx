...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/users/loginUser`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        // Store the token in sessionStorage for added security (cleared when browser is closed)
        sessionStorage.setItem("token", data.token);

        // Dispatch the login action to update Redux state
        dispatch(login({ user: data.user }));

        // Set a cookie with the token for cross-subdomain access if needed
        document.cookie = `authToken=${data.token}; path=/; max-age=86400; secure; samesite=strict`;

        // Check if we have a redirect URL (from author.bignote.com)
        if (redirectUrl) {
          // Create a secure token transfer with expiration
          ...
          // Redirect to author site with token and verification params
          window.location.href = `${redirectUrl}?token=${data.token}&state=${state}&ts=${timestamp}`;
        } else {
          // Regular flow - redirect to Sneat dashboard
          window.location.href =
            "https://react-sneat-bootstrap-admin-template-chi.vercel.app/dashboard";
        }
      } else {
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2 bg-gradient-to-br from-orange-50 to-amber-50">
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            <div className="mb-6 inline-flex items-center gap-2 text-orange-600">
              ...
              <span className="text-2xl font-bold">
                {redirectUrl ? "Authentication Required" : "BigNote Portal"}
              </span>
            </div>
            ...
            <p className="text-gray-600">
              {redirectUrl
                ? "Login to continue to the author dashboard"
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              {loading ? "Signing In..." : "Access Author Dashboard"}
            </button>
          </form>
      ...
      <div className="hidden md:flex bg-gradient-to-br from-[#FF6100] to-amber-600 p-12 items-center justify-center min-h-screen relative overflow-hidden">
        <div className="max-w-2xl text-white text-center space-y-12 z-10">
          ...
          <div className="relative h-[400px] w-full">
            ...
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-4/5 h-3/5 bg-white/5 rounded-t-2xl shadow-xl">
              {/* Laptop Illustration */}
              <div className="absolute left-1/2 -translate-x-1/2 -top-8 w-3/5 bg-white/10 p-4 rounded-lg ">
        ...
        <div className="absolute inset-0 opacity-10">
          <div className="absolute w-full h-full bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjQiIGhlaWdodD0iMjQiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGcgZmlsbD0ibm9uZSIgZmlsbC1ydWxlPSJldmVub2RkIj48cGF0aCBkPSJNMCAwaDI0djI0SDB6Ii8+PHBhdGggZD0iTTEyIDZjMi45NzMgMCA1Ljg5My ')] repeat"></div>
        </div>
      </div>
    </div>
  );
